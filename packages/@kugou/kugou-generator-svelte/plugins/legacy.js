...
export function getLinkInfo(node) {
  let href
  let isModule = false
  for (let i = 0; i < node.props.length; i++) {
    const p = node.props[i]
    if (p.type === NodeTypes.ATTRIBUTE) {
      if (p.name === 'href') {
        href = p
      } else if (p.name === 'rel' && p.value && p.value.content === 'modulepreload') {
        isModule = true
      }
    }
  }
  return { href, isModule }
}
...
const systemJSInlineCode = `
var existingHook = System.constructor.prototype.createScript;
System.constructor.prototype.createScript = function (url) {
  var script = existingHook.call(this, url);

  if (url.indexOf(location.origin) === 0 && location.protocol === 'content:') {
    script.crossOrigin = null;
  }

  return script;
};
System.import(document.getElementById('${legacyEntryId}').getAttribute('data-src'));
`
...
function viteLegacyPlugin(options = {}) {
  ...
  const genChunk = options.genChunk !== false
  ...
  const legacyPostPlugin = {
    ...
    transformIndexHtml(html, { chunk }) {
      ...
      if (modernPolyfillFilename && genChunk) {
        tags.push({
          tag: 'script',
          attrs: {
            type: 'module',
      ...
      if (genChunk) return { html, tags }

      const s = new MagicString(html)

      traverseHtml(html, chunk.fileName, (node) => {
        ...
        // script tags
        if (node.tag === 'script') {
          const { isModule } = getScriptInfo(node)

          shouldRemove = isModule
        } else if (node.tag === 'link') {
          const { isModule } = getLinkInfo(node)
          shouldRemove = isModule
        }
